...
# Get the Terraform output value for 'server_linux_ip'
...
misp_private_ip = get_terraform_output("misp_private_ip")
indexer_username = "admin@admin.test"
indexer_password = "admin"


# Step 2: Authenticate using curl and get token
def get_auth_token(username, password, misp_private_ip):
    curl_command = [
        "curl",
        "-u", f"{username}:{password}",
        "-k", 
        "-X", "POST",
        f"https://{misp_private_ip}:55000/security/user/authenticate?raw=true"
    ]
    ...
    if result.returncode == 0:
        ...
        print(f"Error in authentication: {result.stderr}")
        return None


# Get the token from authentication
token = get_auth_token(indexer_username, indexer_password, misp_private_ip)

if token:
    print(f"Successfully authenticated. Token: {token}")
    
    # Step 3: Use the token as the MISP API key
    misp_api_auth_key = token  # Use the token as your API key
    
    # Step 4: Make an API request using the token
    misp_base_url = f"https://{misp_private_ip}/attributes/restSearch/"
    headers = {
...
misp_base_url = "https://${{misp_private_ip}}/attributes/restSearch/"
# MISP Server API AUTH KEY
misp_api_auth_key = {token}
# API - HTTP Headers
